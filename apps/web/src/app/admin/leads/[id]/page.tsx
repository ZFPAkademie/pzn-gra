    ...
    <div className="min-h-screen">
      ...
      <header className="bg-white shadow-sm">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <Link
            href="/admin/leads"
            className="text-sm text-stone-700 hover:text-navy mb-2 inline-block"
          >
            ← Zpět na inbox
          </Link>
        ...
        <div className="grid gap-6 md:grid-cols-2">
            ...
            <dl className="space-y-3">
              ...
              {lead.apartment_title && (
                <div>
                  ...
                  <dd className="text-navy">{lead.apartment_title}</dd>
                </div>
              )}
              {lead.preferred_dates && (
                <div>
                  <dt className="text-sm text-slate-500">Preferované termíny</dt>
                  <dd className="text-navy">{lead.preferred_dates}</dd>
                </div>
              )}
              {lead.guest_count && (
                <div>
                  <dt className="text-sm text-slate-500">Počet hostů</dt>
                  <dd className="text-navy">{lead.guest_count}</dd>
                </div>
              )}
              <div>
                <dt className="text-sm text-slate-500">Vytvořeno</dt>
                <dd className="text-navy">{formatDate(lead.created_at)}</dd>
              </div>
            </dl>
          </div>
          ...
          {lead.message && (
            <div className="bg-white rounded-lg shadow p-6 md:col-span-2">
              ...
              <div className="bg-stone rounded p-4 text-slate-700 whitespace-pre-wrap">
                {lead.message}
              </div>
            </div>
          )}
          ...
          <div className="bg-white rounded-lg shadow p-6 md:col-span-2">
            ...
            <dl className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <dd className="font-mono text-xs text-stone-700">{lead.id}</dd>
              </div>
              <div>
                <dt className="text-slate-500">Jazyk</dt>
                <dd className="text-navy">{lead.language?.toUpperCase()}</dd>
              </div>
              <div>
                <dt className="text-slate-500">GDPR souhlas</dt>
                <dd className="text-navy">{lead.gdpr_consent ? '✓ Ano' : '✗ Ne'}</dd>
              </div>
              <div>
                <dt className="text-slate-500">Marketing souhlas</dt>
                <dd className="text-navy">{lead.marketing_consent ? '✓ Ano' : '✗ Ne'}</dd>
              </div>
              {lead.ip_address && (
                <div>
                  <dt className="text-slate-500">IP adresa</dt>
                  <dd className="font-mono text-xs text-stone-700">{lead.ip_address}</dd>
                </div>
              )}
              <div>
                <dt className="text-slate-500">Poslední aktualizace</dt>
                <dd className="text-navy">{formatDate(lead.updated_at)}</dd>
              </div>
            </dl>
          </div>
        </div>
